module Blog
  class PostsController < Blog::ApplicationController
    ...
    def create
      ...
      @post.publish
      if @post.save
        redirect_to permalink_path(@post), notice: 'Post was successfully created.'
      else
        render :action => :new
      end
    end
    ...
    def update
      ...
      if @post.save
        redirect_to permalink_path(@post), notice: 'Post was successfully updated.'
      else
        render :action => :edit
      end
    end
    ...
    def load_by_permalink
      ...
      @post = Post.where(published_at: (date..date.end_of_day), permalink: params[:permalink]).first!
    end

    def comments_to_display
      if can?(:update, Blog::Comment)
        @post.comments
      else
